...
4. `docs/maintenance/CURRENT.md`：当前源码、版本、证据、限制和下一项最有价值的验证。
5. 按任务读取 `README.md`、相关测试、相关平台探针和必要的历史条目；不要默认读取历史归档。
...
验证入口：`test/docs-check.cjs` 维护本知识树的大小、链接、版本和关键内容门禁；受影响平台的
...
| 任务 | 必读节点 | 只在必要时读取 |
|---|---|---|
| 普通代码修复 | `AGENTS.md`、本文件、`MAINTENANCE_WORKFLOW.md`、`maintenance/CURRENT.md` | 受影响平台的源码/测试、README 对应段落 |
| 平台适配或真实站点验证 | 上述四项 | 对应 `test/real-*.cjs`、平台回放、相关历史条目 |
| 性能、日志、生命周期 | 上述四项 | `test/performance.cjs`、运行时相关测试、性能历史 |
| 用户可见功能 | 上述四项、`README.md` | 当前版本 changelog 条目、受影响测试 |
| 发布或回滚 | 上述四项、`CHANGELOG.md`、`docs/changelog/INDEX.md` | 对应版本条目和发布历史 |
