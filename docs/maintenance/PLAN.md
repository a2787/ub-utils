...
- status: in_progress
- stage: 阶段 A-D 已完成；阶段 E 已完成当前 loopback 模型快照的固定配置评测，字幕/音频/画面语义与真实内容泛化仍保持 `blocked`
- priority: P1
...
- acceptance: required
  ...
  - [x] 结构预算：产品请求上下文相对当前基线增加 `19.8%`，不超过 25%；指标不得通过全部延后候选取得。
  - [x] 当前 loopback 网关的人工合成固定配置评测：baseline/work/full 各 3 次，full/baseline p95 为 `1.048`，低于 `1.30`；schema 错误、未核查事实误阻断和身份越权动作均为 `0`。
  - [ ] 单作品生产 payload 的输入预算与长期 p95：本轮混合 24 条数据的一次性 full payload 增长 `81.6%`，不代表生产单作品请求，需另做单作品/大样本基线；真实内容精度也未纳入本地合成门禁。
  - [x] 通过 B 站 AI/弹幕/通用回归、文档门禁和真实只读探针；未具备稳定字幕/音频证据时明确标记 `blocked`，不声称理解视频声音。
- evidence: `structure regression`（CTX-1..9、AI/桥接/平台/通用矩阵和本机评测门禁）；`real-site verified`（2026-09-10 匿名隔离 B 站只读探针，0.55.0，34+34 条上下文记录，背景确认后当前作品作用域立即生效且全局键不变；来源探针确认标准字幕轨道为 0、播放器仅有媒体存在证据）；`blocked`（真实内容泛化精度、单作品生产预算、字幕/音频/画面语义）。计划说明见[实施方案](plans/2026-09-10-ob-ai-014.md)。
- next: 保留当前 loopback 数字作为模型快照基线；下一步按单作品请求拆分评测并在模型别名变化时重跑，继续只读探索字幕来源，未取得稳定可读输入前不接入生产路径。
- updated: 2026-09-10
- supersedes: none
