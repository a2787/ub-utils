...
- 候选状态：`OB-WEIBO-003` 作品级评论读取/滚动锁恢复与 `OB-UI-001` 通用悬浮入口收敛均已通过用户专用浏览器体验，准备完成本地收尾；当前仍未公开发布。
