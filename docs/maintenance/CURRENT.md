...
- 候选状态：`acd3b0a` 已提交本地功能修复；0.46.2 未 push、未创建 tag 或 Release。
...
- 当前状态：0.46.2 B站候选已提交本地源码与回归测试，未公开发布；当前用户 Chrome 的候选后置安装仍受浏览器安全策略阻断，需安装候选后刷新页面再观察。
...
- 当前状态：功能与测试提交为 `acd3b0a`；本条文档同步随本地候选一起提交，未 push、未创建 tag 或 Release。
