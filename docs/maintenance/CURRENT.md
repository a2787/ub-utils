...
更新时间：2026-09-05
状态来源：0.46.1 B站评论菜单回归候选；0.46.0 仍是公开版本，历史过程见 [HISTORY_INDEX.md](HISTORY_INDEX.md)。

## 当前版本

- 当前 userscript：`0.46.1`（本地候选）
- 构建：`0.46.1-bili-comment-menu-report`
- 当前公开版本：`0.46.0`
- 当前公开功能提交：`75ba0f7c7c54a056f0e41e9ae39ddd5a6340e770`
- 最近验证的源码快照：`7f46ecaefe25228de2d1278beccb2ceaaf5af49f`
- 当前候选源码 SHA-256：`0e7177c683fb31962a497119cf5277eab3a9f25f90e84c894cf674001080e694`
- 候选状态：`7f46eca` 已提交本地功能修复；0.46.1 未 push、未创建 tag 或 Release。
...
- 0.46.1 候选结构回归：通用 20、核心 9、B站 34、自动弹幕 7、评论管理器 3、作品级 3、性能 8、开发扩展 4、适配器 28、微博回放 13，合计 129/129；
  `node --check omniblock.user.js`、`node test/docs-check.cjs` 和 `git diff --check` 通过，控制台/页面错误为 0。
- 0.46.0 的旧平台真实站点细节仍按各自 dated 条目保留；本轮未把未重跑的其他平台线上功能自动升级为 0.46.1 证据。

### `real-site verified`

- B站 0.46.1 候选的日期、脱敏页面形式、菜单/作者/整楼结果见上方 `2026-09-05 B站评论举报菜单回归` 条目；候选隔离页未读取 Cookie，未触发平台写入。
- 其他平台的登录态结果、页面总量与用户体验均保留在对应 dated 条目；它们不因本次 B站补丁自动升级为 0.46.1 证据。

### `blocked`

- 当前用户 Chrome 本轮不能刷新 0.46.1：浏览器安全策略阻止打开 `chrome://extensions`；因此同一已安装实例的候选后置观察留给用户安装后执行。
...
浏览器安全策略阻止本轮从 `chrome://extensions` 刷新候选，后续安装 0.46.1 后需刷新目标平台页面；0.46.1 尚未公开发布。

## 下一项最有价值的验证

下一项最有价值的验证是由用户安装 0.46.1 后在同一 B站视频页刷新并实际点开评论三点菜单；随后再取得稳定的抖音换片目标并区分插件自身扫描/脚本时长与页面 renderer 总量，同时为贴吧现代 `.thread-card` 建立只读身份契约探针。微博顶层 spacer 和 B站根评论分页仍需各自可测样本；后续版本的 CI/CD、push、tag 和 Release 仍需当轮明确授权。
