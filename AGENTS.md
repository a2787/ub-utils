...
3. 每轮实现必须先选择或创建 `docs/maintenance/PLAN.md` 中的 `OB-*` 计划项，写明范围、
   非目标、验收标准和下一步；没有计划 ID 的大改动不得开始。计划是执行依据，
   不是重复授权门槛：用户已经明确授权、范围清楚且只涉及本地可回滚实现时，完成
   分析和计划后直接执行，不再等待用户再次确认。
...
- AI 只能提出候选；只有用户在审核界面明确确认后，才允许写入本地名单，绝不调用平台官方拉黑、举报、关注或发帖。
- “没有引用/没有来源/尚未核查”不等于“内容为假”：事实性主张在缺少正面矛盾证据或上下文不足时必须延后，不得仅凭模型臆测生成候选；规则违规或有正面矛盾证据才可进入人工审核。
- `bili:dmhash` 永远不能伪装成已知 UID。只有候选集合唯一且 B 站用户卡片正向校验成功才补 UID；碰撞、失败、超时、路由/会话失效一律保留 hash-only 并丢弃迟到结果。
- 屏蔽确认后的基础 hash/已有 UID 可先本地生效，UID 补充必须是可撤销、可暂停、可按页面会话取消的后台增强；停用、隐藏、换路由和运行时销毁都必须关闭旧任务。
- 当前版本不接入公共检索或外部来源；未来事实检索必须另立计划，先评审来源白名单、隐私脱敏、缓存 TTL、失败回退、成本和离线评测门槛，不能把检索当作本轮实现的隐含扩权。
- AI 证据只允许标记为 `structure regression`、`real-site verified` 或 `blocked`；夹具通过不能升级为真实站点或模型精度结论。

## 规则治理

规则服务于核心需求和可验证性，不得机械执行已经过时或与用户明确目标冲突的条款。
发现冲突时必须先向用户说明具体矛盾、风险和拟议修订，并在同一工作中更新本文件后再
按新规则执行；不得静默绕过，也不得借“灵活”降低身份、测试或证据标准。对于用户
已经明确授权的任务，只有触及删除/覆盖重要数据、密钥或 `.env`、数据库迁移、系统
配置、git 历史或公开发布、平台写入、真实外部副作用，或需要扩大任务范围时，才需要
暂停并重新请求方向；普通的实现细节不应再次要求确认。
...
6. push、公开 tag、GitHub Release、部署和其他公开发布必须得到当前任务上下文中的明确授权；
   历史对话、旧交接或“常规版本发布”约定不自动继承。用户在当前项目上下文明确说“推送版本”时，
   该短语视为对本轮正常版本交付链的完整授权：选择性 commit、推送当前分支、创建与 userscript
   `@version` 一致的 tag、创建 GitHub Release，以及执行项目已有的登记部署/重启链。此默认授权不包含
   平台举报/拉黑/关注/发帖等平台写入、读取或代输凭证、删除/覆盖重要数据、数据库迁移、修改 `.env`
   或密钥、系统配置、修改远端/更新 URL、覆盖既有 tag/Release 等实质不同操作；未取得相应授权时只能
   保留本地提交或候选产物。
