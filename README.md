...
- **B站视频弹幕**（从拦截或主动只读获取的数据段 `seg.so` 建立当前视频发送者列表，
  支持单条/勾选批量本地屏蔽，并按 `mid_hash` 过滤；评论 UID 也会经 CRC32 自动映射）
...
- B站视频页右下角始终显示 **「弹幕屏蔽(N)」**；数据尚未就绪时显示 `(0)`，脚本会主动
  读取当前视频首段，失败时面板内可点「重新读取」。打开后可按弹幕内容搜索、单独屏蔽
  发送者，或勾选多人后批量屏蔽；不依赖移动弹幕的举报菜单或右侧原生列表，并兼容
  PAKKU 接管播放器弹幕请求的加载顺序。
...
- **B站弹幕**：固定使用右下角「弹幕屏蔽(N)」工具展示脚本已从弹幕段解析出的发送者；
  ...
  只读获取首段兜底。单条按钮和勾选批量入口都保存规范的 `bili:dmhash:<hash>`，不会把
  不可逆 hash 伪装成 UID。
...
- **B站弹幕**在页面拿到数据前拦截实际的 `seg.so` / `list.so` 弹幕段，并在错过首段时按
  ...
  经 CRC32 映射，或工具直接保存 `bili:dmhash:<hash>`；不会改写 `view` 元数据响应。
...
|---|---|---|
| B站 | `real-site verified`：2026-08-21 隔离未登录访问 `BV1eyYRz2E2v`，识别 2 条根评论和 4 条楼中楼、批量计数为 6；拉黑一条楼中楼后仅该回复高度归零，根线程保留，撤销恢复。真实页面同时观察到主动 `fetch` 与播放器 XHR 的 `seg.so`，工具去重列出 7 位发送者，单条与两人批量屏蔽及撤销均成功。 | PAKKU 前后两种注入顺序依据其公开源码做了 `structure regression`，尚未在隔离探针中安装真实 PAKKU 扩展。原生移动弹幕菜单和右侧列表仍无安全身份行；已进播放器缓存的弹幕需下一段加载或刷新后才完全消失。 |
...
node test/quickblock.cjs          # B站楼中楼、批量事务、弹幕工具、PAKKU 契约与 XHR 共 19 项回归
