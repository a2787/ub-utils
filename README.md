...
- B站视频页右下角始终显示 **「弹幕屏蔽(N)」**；数据尚未就绪时显示 `(0)`，脚本会主动
  ...
  会查询可能发送者；打开候选主页核对后点「确认并拉黑」，名单才会同时保存 hash 与 UID，
  此后该 UID 的评论和对应 hash 的弹幕都会隐藏。
...
|---|---|---|
| B站 | `real-site verified`：2026-08-23 隔离未登录、自动发现的公开 `bilibili.com/video/...` 页。播放器渲染 3 条浮动弹幕，全部为 `pointer-events: none`；指针坐标命中后跟随浮层出现，拉黑写入 `bili:dmhash:` 并可撤销。同轮识别 4 位评论作者并验证评论入口；弹幕工具列出 26 组/18 位发送者，单组与批量事务均成功。 | 右侧弹幕列表本轮未展开（`danmakuRowCount=0`），行内入口仍为 `structure regression`；未登录会话下原生弹幕「举报」操作条不可见，统一「本地拉黑」路径为 `blocked`。UID 候选碰撞边界为 `structure regression`；普通弹幕段不直接提供 UID，反查只覆盖 1–10 位数字且无法自动消除 CRC32 碰撞；真实 PAKKU 扩展共存尚未验证。 |
...
node test/quickblock.cjs          # B站楼中楼、弹幕分组、列表分段、浮动弹幕坐标命中、UID 候选、PAKKU 与 XHR 共 26 项回归
