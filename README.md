...
| 微博 | `real-site verified`：2026-08-24 隔离未登录、自动发现的公开 `weibo.com/...` 详情页观察到「本地拉黑作者」；22 条评论中 17 条根评论、5 条楼中楼，21 条解析出身份、26 个行内入口可见。此前 2026-08-23 的页面中 28 条评论有 21/21 根评论和 6/6 条作者不同于根评论的已加载楼中楼入口，回复隐藏/撤销也通过。 | 本轮选中的回复虽能打开确认框并隐藏，但虚拟列表在撤销阶段回收根评论，完整“根评论保持可见且撤销恢复”记为 `blocked`；「共 N 条回复」弹窗结构为 `structure regression`，已登录虚拟列表、其他微博前端变体和更深层回复仍需逐结构验证。未登录会话看不到点赞/转发用户列表弹窗，单人弹窗批量仍只有结构回归证据。 |
