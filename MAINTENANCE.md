...
- `structure regression`：新增的通用浏览器回归会要求当前源码的 `@version`、`RUNTIME_BUILD`、
  `window.OB.runtime`、齿轮属性和设置面板文本一致；候选代码完成矩阵后再记录确切结果。
...
- 本条改动完成后必须运行 `node --check omniblock.user.js`、`node test/run.cjs`、完整受影响矩阵、
  `git diff --check` 和暂存前隐私门禁；不得把候选尚未安装写成发布完成。
