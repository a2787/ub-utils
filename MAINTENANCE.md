...
- `test/real-login-probe.cjs`：增加 `--scenario=feed`、`--disable-quick-block`、`--target-id=` 和固定桌面视口；
  ...
  重复测试时叠加初始化脚本。
- `README.md`、`CHANGELOG.md`：补充候选行为、验证边界和未发布状态。
...
- `0.41.0` 已完成候选验证，尚未提交、推送、打 tag 或创建 Release；当前已发布版本仍为 `v0.40.0`。候选没有安装
  到用户主浏览器，专用浏览器中的直接注入也不改变 Tampermonkey 已安装版本。

**下一步验证**

完成隐私门禁和暂存差异审阅后，提交、推送、创建 `v0.41.0` tag 与 GitHub Release；发布后继续保留专用 Chrome
作为后续微博结构变化的只读回归入口，主浏览器由用户自行更新和最终体验验收。
